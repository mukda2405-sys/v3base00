...
export class MetricsStore {
	...
	async getInstance(instanceId: string): Promise<HeartbeatSnapshot | null> {
		try {
			const row = await this.db.prepare(`SELECT instance_id, timestamp, hashrate, cpu_percent, pool, connection_status, started_at, updated_at FROM instance_latest WHERE instance_id = ? LIMIT 1`).bind(instanceId).first<Record<string, unknown>>();
	...
	async getActiveInstances(): Promise<HeartbeatSnapshot[]> {
		try {
			...
			const r = await this.db.prepare(`SELECT instance_id, timestamp, hashrate, cpu_percent, pool, connection_status, started_at, updated_at FROM instance_latest WHERE updated_at >= ? ORDER BY hashrate DESC`).bind(cutoff).all<Record<string, unknown>>();
	...
	async getHourly(limitHours = 24): Promise<HourlyBucket[]> {
		...
		try {
			const r = await this.db.prepare(`SELECT hour, total_instances, avg_hashrate, peak_hashrate FROM hourly_stats ORDER BY hour DESC LIMIT ?`).bind(safeLimit).all<Record<string, unknown>>();
