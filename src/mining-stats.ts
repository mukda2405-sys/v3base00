...
export class MiningStatsStore {
	...
	async recordStatsBatch(payloads: MiningStatsPayload[]): Promise<void> {
		...
		for(let i = 0; i < payloads.length; i += STATEMENT_LIMIT){
			...
			const statements = chunk.map((payload) => {
				...
				return this.db.prepare(`INSERT INTO instance_latest (instance_id, timestamp, hashrate, cpu_percent, pool, connection_status, updated_at, started_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(instance_id) DO UPDATE SET timestamp = CASE WHEN excluded.timestamp >= instance_latest.timestamp THEN excluded.timestamp ELSE instance_latest.timestamp END, hashrate = CASE WHEN excluded.timestamp >= instance_latest.timestamp THEN excluded.hashrate ELSE instance_latest.hashrate END, cpu_percent = CASE WHEN excluded.timestamp >= instance_latest.timestamp THEN excluded.cpu_percent ELSE instance_latest.cpu_percent END, pool = CASE WHEN excluded.timestamp >= instance_latest.timestamp THEN excluded.pool ELSE instance_latest.pool END, connection_status = CASE WHEN excluded.timestamp >= instance_latest.timestamp THEN excluded.connection_status ELSE instance_latest.connection_status END, updated_at = CASE WHEN excluded.timestamp >= instance_latest.timestamp THEN excluded.updated_at ELSE instance_latest.updated_at END, started_at = CASE WHEN COALESCE(instance_latest.started_at, 0) > 0 THEN instance_latest.started_at ELSE excluded.started_at END`).bind(cleanText(payload.instanceId, "unknown", 128), timestamp, toNonNegativeNumber(payload.hashrate), toNullablePercent(payload.cpuPercent), cleanNullableText(payload.pool, 256), cleanNullableText(payload.connectionStatus, 64), now, timestamp);
			});
	...
	async getHistory(limit = 100): Promise<unknown[]> {
		...
		const r = await this.db.prepare(`SELECT timestamp, '__cluster__' AS instance_id, total_hashrate AS hashrate, active_instances, running_instances, total_instances, average_hashrate, peak_hashrate, cumulative_uptime_seconds, operation FROM mining_status_reports ORDER BY timestamp DESC LIMIT ?`).bind(safeLimit).all();
		return r.results ?? [];
	}
	...
	async getTotals(): Promise<MiningTotals> {
		...
		const row = await this.db.prepare(`SELECT COALESCE(SUM(hashrate), 0) AS totalHashrate, COALESCE(AVG(CASE WHEN hashrate > 0 THEN hashrate END), 0) AS averageHashrate, COALESCE(MAX(hashrate), 0) AS peakHashrate, COALESCE(SUM(CASE WHEN hashrate > 0 AND updated_at > started_at THEN updated_at - started_at ELSE 0 END), 0) AS cumulativeUptimeMs, COUNT(*) AS totalRecords, COALESCE(SUM(CASE WHEN hashrate > 0 THEN 1 ELSE 0 END), 0) AS activeInstances FROM instance_latest WHERE updated_at >= ?`).bind(staleCutoff).first<Record<string, unknown>>();
		...
		return {
			...
			cumulativeUptimeSeconds: Math.floor(num("cumulativeUptimeMs") / 1000),
	...
	async writeStatusReport(coordinatorStatus?: {
		...
		await this.db.prepare(`INSERT INTO mining_status_reports (timestamp, total_instances, running_instances, stopped_instances, active_instances, total_records, total_hashrate, average_hashrate, peak_hashrate, cumulative_uptime_seconds, operation, config_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(timestamp, totalInstances, runningInstances, stoppedInstances, totals.activeInstances, totals.totalRecords, totals.totalHashrate, totals.averageHashrate, totals.peakHashrate, totals.cumulativeUptimeSeconds, operation, configJson).run();
	...
	async rollupHourly(hour?: number): Promise<number> {
		...
		const r = await this.db.prepare(`INSERT INTO hourly_stats (hour, total_instances, avg_hashrate, peak_hashrate, avg_cpu_percent) SELECT ?, COALESCE(MAX(total_instances), 0), COALESCE(AVG(total_hashrate), 0), COALESCE(MAX(total_hashrate), 0), 0 FROM mining_status_reports WHERE timestamp >= ? AND timestamp < ? HAVING COUNT(*) > 0 ON CONFLICT (hour) DO UPDATE SET total_instances = excluded.total_instances, avg_hashrate = excluded.avg_hashrate, peak_hashrate = excluded.peak_hashrate, avg_cpu_percent = excluded.avg_cpu_percent`).bind(targetHour, targetHour, nextHour).run();
...
async function ensureSchema(db: D1Like): Promise<void> {
	...
	await db.batch([
		db.prepare(`CREATE TABLE IF NOT EXISTS instance_latest ( instance_id TEXT PRIMARY KEY, timestamp INTEGER NOT NULL, hashrate REAL NOT NULL DEFAULT 0, cpu_percent REAL, pool TEXT, connection_status TEXT, updated_at INTEGER NOT NULL, started_at INTEGER NOT NULL DEFAULT 0 )`),
		...
		if(dataset && typeof dataset.writeDataPoint === "function"){
			for(const p of payloads){
				dataset.writeDataPoint({
					...
					blobs: [typeof p.instanceId === "string" ? p.instanceId.slice(0, 128) : "unknown", typeof p.pool === "string" ? p.pool.slice(0, 256) : "", typeof p.connectionStatus === "string" ? p.connectionStatus.slice(0, 64) : "", colo ?? "", typeof p.poolState === "string" ? p.poolState.slice(0, 64) : "", typeof p.tlsStatus === "string" ? p.tlsStatus.slice(0, 64) : ""],
					doubles: [Number(p.hashrate) || 0, Number(p.cpuPercent) || 0, Number(p.timestamp) || receivedAt],
				});
			}
		}
	}catch(err){
...
function heartbeatLog(level: "warn" | "error", fields: Record<string, unknown>, msg: string): void {
