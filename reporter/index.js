	...
	fetchXmrigApi("/1/summary", (err, summary) => {
		if(err){
			...
			if(reporterUptimeSec > LIVENESS_GRACE_SEC){
				...
				console.warn(`[reporter] api-unreachable sample ${consecutiveZeroHashSamples}/${STALL_THRESHOLD} (reporterUptime=${reporterUptimeSec.toFixed(0)}s, err=${err.message})`);
				if(consecutiveZeroHashSamples >= STALL_THRESHOLD){
					console.error(`[reporter] XMRig API STALLED: unreachable for ${STALL_THRESHOLD} consecutive samples. Exiting to force container restart.`);
					process.exit(2);
				}
			}
			return;
