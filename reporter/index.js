...
process.on("unhandledRejection", (reason, promise) => {
	console.error(
		"[reporter] UNHANDLED REJECTION at:",
		promise,
		"reason:",
		reason,
	);
...
function parseXmrigLog() {
	...
	for (const line of lines) {
		...
		if (
			...
			tlsStatus = "error";
			if (
				line.includes("handshake failed") ||
				line.includes("handshake error")
			) {
		...
		if (
			...
			line.includes("write error") ||
			line.includes("socket error")
		) {
			poolState = "disconnected";
			lastError = line.trim();
			lastErrorTime = Date.now();
	...
	fetchXmrigApi("/1/summary", (err, summary) => {
		...
		getCpuUsage((cpuPercent) => {
			getMemoryUsage((memoryPercent) => {
				...
				const reporterUptimeSec = process.uptime();
				if (
					reporterUptimeSec > LIVENESS_GRACE_SEC &&
					(!hashrate || hashrate <= 0)
				) {
					consecutiveZeroHashSamples++;
					console.warn(
...
function reportMetrics() {
	...
	const requeue = (label) => {
		...
		settled = true;
		console.error(
			`[reporter] ${label}; requeuing ${pendingBuffer.length} samples`,
		);
		statsBatchBuffer = pendingBuffer
			.concat(statsBatchBuffer)
		...
		(res) => {
			if (res.statusCode === 200) {
				...
				if (Date.now() - lastSuccessfulPushLogAt > 600000) {
					...
					console.log(
						`[reporter] Metrics push successful (${batchSize} samples)`,
					);
				}
			} else {
				res.resume();
	...
	if (req.url === "/pool-status" && req.method === "GET") {
		...
		res.end(
			JSON.stringify({
				...
				lastErrorTime: cachedStats.lastErrorTime,
				lastErrorAgo:
					cachedStats.lastErrorTime > 0
						? Date.now() - cachedStats.lastErrorTime
						: null,
...
server.listen(PORT, () => {
	...
	console.log(
		`[reporter] Endpoints: /health, /stats, /xmrig-summary, /logs, /pool-status`,
	);
});
